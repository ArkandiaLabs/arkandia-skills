...
Check for `AGENTS.md` at the repo root and a `docs/` directory with `.md` files.

- **Base pack present → companion mode.** You will add `docs/dotnet.md` and **enrich** the
  existing docs (Phase 3).
- **Base pack absent → graceful-fallback mode.** Do **not** dead-end, and do **not** generate the
  full base pack (that is `agent-context`'s job, and copying its templates here would duplicate
  them). Instead you will produce `docs/dotnet.md` plus a **minimal `AGENTS.md` stub + `CLAUDE.md`
  delegator generated inline** (Phase 3), so the deep-dive is reachable. Tell the user once:
...
Resolve the output language (Phase 2 asks if `$ARGUMENTS` was empty).
...
Do **not** copy base templates. Generate inline:

- `AGENTS.md` — ~10 lines: project name + one-line purpose; a "Where to find things" section whose
  only entry is `docs/dotnet.md` ("Deep .NET context"); a short "Security" section (no secrets
...
Skip 3b (those docs don't exist).
