...
- Primary language(s), framework, and **version** (for .NET: target framework moniker per project).
- Persistence: database engine + provider/driver + migration tool (for .NET: EF Core provider).
- Deployment target and CI system.
- The 3–5 most-run developer commands.
- Key domain entities and their relationships.
- The user-supplied non-obvious rules / invariants (for .NET: project-layering rules, DI lifetimes).
...
Example: `"The API targets .NET 8."` → source `src/Api/Api.csproj:6`, confidence `high`.
...
|---|---|---|---|
| The API targets .NET 8. | `src/Api/Api.csproj:6` | high | confirmed |
...
|---|---|---|---|
| La API usa .NET 8. | `src/Api/Api.csproj:6` | alta | confirmada |
