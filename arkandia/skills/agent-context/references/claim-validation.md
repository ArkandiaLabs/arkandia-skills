...
- Primary language(s), framework, and **version**.
- Persistence: database engine + provider/driver + migration tool.
- Deployment target and CI system.
- The 3–5 most-run developer commands.
- Key domain entities and their relationships.
- The user-supplied non-obvious rules / invariants.
...
Example: `"The service targets Java 21."` → source `pom.xml:14`, confidence `high`.

## Step 3 — Flag ambiguity

Mark a claim **ambiguous** when:

- More than one interpretation is plausible (e.g. two manifests pin different versions; an entity
  name is overloaded).
...
Generated by the agent-context skills. Records the load-bearing factual claims in the docs, their
source in the repository, and whether a human confirmed them. Re-run the skill to refresh.

| Claim | Source | Confidence | Status |
|---|---|---|---|
| The service targets Java 21. | `pom.xml:14` | high | confirmed |
| Persistence is PostgreSQL via Flyway. | inferred | medium | corrected → MySQL |
...
Generado por las skills agent-context. Registra las afirmaciones factuales clave de la
...
|---|---|---|---|
| El servicio usa Java 21. | `pom.xml:14` | alta | confirmada |
| La persistencia es PostgreSQL vía Flyway. | inferido | media | corregida → MySQL |
